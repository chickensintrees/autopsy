...
Write-Host ""
Write-Host "Optional - enforce the banner relay with a Stop hook (CLI/desktop):"
Write-Host "  $Py $ScriptDir\hooks\enable.py         # idempotent, backs up settings.json"
Write-Host "  $Py $ScriptDir\hooks\enable.py --dry-run   # preview the change first"
